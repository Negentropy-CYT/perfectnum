...
def _structure_lines(
    ...
    *,
    ...
    if s.obligation_signatures:
        ot = sum(s.obligation_signatures.values())
        uniq = len(s.obligation_signatures)
        top10 = sum(v for _, v in s.obligation_signatures.most_common(10))
        w("\n## Obligation recurrence")
        w(f"  unique/total: {uniq:,} / {ot:,}  ({100.0*uniq/ot:.2f}%)")
        w(f"  top-10 coverage: {top10:,}  ({100.0*top10/ot:.1f}%)")
        w("  Top-10 signatures:")
        for (pending, nf, coarse), count in s.obligation_signatures.most_common(10):
            w(f"    pending={set(pending)} |f|={nf} "
              f"h~=1e-{coarse}  x{count:,}")
    ...
    if s.obligation_signatures and _sig_factors is not None:
        lines.extend(
            _attractor_lines(s, _sig_factors, max_prime)
        )
...
def _attractor_lines(s, _sig_factors, max_prime) -> List[str]:
    """Build attractor-source-closability text section."""
    lines: List[str] = []
    w = lines.append

    attractor_primes: Dict[int, int] = {}
    for (pending, nf, _coarse), count in s.obligation_signatures.most_common(10):
        for q in pending:
    ...
    w(f"  {'obligation':>14}  {'freq':>10}  {'in pool':>8}  "
